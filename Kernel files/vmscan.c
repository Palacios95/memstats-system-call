...
int is_page_referenced(struct page* page)
{
        //If ref_count > 0, decrement count and return 1 (page is referenced).
        if(page->ref_count > 0)
        {
                page->ref_count--;
                return 1;
	...
	//Page not referenced. Return 0.
        return 0;
}

//ADDED BY ALEJANDRO PALACIOS
void mutate_ref_count(struct page* page)
{
				...
				struct zone *zone, struct scan_control *sc)
{
	...
	//ADDED BY ALEJANDRO PALACIOS
	zone->inactive_evicted_count += nr_reclaimed;
...
force_reclaim_mapped:
	...
	while (!list_empty(&l_hold)) 
	{
		...
		list_del(&page->lru);
		//Mutates the reference count before determining to send to inactive list or not.
		mutate_ref_count(page);
		if (page_mapped(page)) 
		{
			if (!reclaim_mapped ||
			    (total_swap_pages == 0 && PageAnon(page)) ||
			    is_page_referenced(page)/*new check that replaced the old page_referenced function*/) 
			{
	...
	*/
	...
	//ADDED BY ALEJANDRO PALACIOS;
	zone->active_inactive_count += pgdeactivate;
